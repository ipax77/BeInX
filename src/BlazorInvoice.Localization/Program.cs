...
class Program
{
    ...
    {
        var match = Regex.Match(resourceName, @"\.([^\.]+)\.csv");
        if (!match.Success)
        {
            return;
        }
        ...
        var defaultLang = Path.Combine(Environment.CurrentDirectory, $"{locName}.en.resx");
        var defaultFile = Path.Combine(Environment.CurrentDirectory, $"{locName}.resx");
